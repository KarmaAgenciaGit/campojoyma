-- Ajusta el filtro del RPC de cambios para usar fecha y hora de llegada (created_at).
...
begin
  ...
  filtered as (
    ...
    where (
      ...
      and (p_fecha_pedido_from is null or c.created_at >= p_fecha_pedido_from)
      and (p_fecha_pedido_to is null or c.created_at <= p_fecha_pedido_to)
...
is 'Devuelve una página de cambios agrupada por PDF sin partir grupos, incluyendo fila meta con totales y filtro por fecha/hora de llegada (created_at).';
