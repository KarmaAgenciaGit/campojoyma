-- Paginación por grupos de PDF para pedidos/previsiones y cambios.
-- Devuelve siempre una fila "meta" con totales + filas "item" con los registros de la página.

create index if not exists idx_pedidos_tipo_pdf_fecha
  on public.pedidos (tipo_pedido, archivo_pdf_id, fecha_pedido desc, created_at desc);

create index if not exists idx_pedidos_tipo_referencia
  on public.pedidos (tipo_pedido, referencia_cliente);

create index if not exists idx_pedidos_tipo_compuesta
  on public.pedidos (tipo_pedido, clienteid, sujetodomicilioid_destino, fecha_carga);

create index if not exists idx_pedidos_tipo_orizon
  on public.pedidos (tipo_pedido, idpedido_orizon);

create index if not exists idx_cambios_tipo_pdf_fecha
  on public.cambios_pedidos (tipo_pedido, archivo_pdf_id, created_at desc);

create index if not exists idx_cambios_tipo_referencia
  on public.cambios_pedidos (tipo_pedido, referencia_cliente);

create index if not exists idx_cambios_tipo_compuesta
  on public.cambios_pedidos (tipo_pedido, clienteid, sujetodomicilioid_destino, fecha_carga);

create index if not exists idx_cambios_tipo_orizon
  on public.cambios_pedidos (tipo_pedido, idpedido_orizon);
...
begin
  ...
  with filtered as (
    ...
    where p.tipo_pedido = p_tipo_pedido
      ...
      and (
        not coalesce(p_en_orizon, false)
        or p.idpedido_orizon is not null
        or p.pedidoclienteid is not null
      )
      and (
  ...
  order by
    case when row_type = 'meta' then 0 else 1 end,
    group_rank asc,
    case when row_type = 'item' and v_order = 'asc' then row_sort_date end asc nulls first,
    case when row_type = 'item' and v_order <> 'asc' then row_sort_date end desc nulls last;
end;
$$;

comment on function public.get_pedidos_group_page(
  text, integer, integer, text, text, bigint, bigint, date, date, date, date, boolean, boolean, boolean, boolean
)
is 'Devuelve una página de pedidos/previsiones agrupada por PDF sin partir grupos, incluyendo fila meta con totales.';
...
begin
  ...
  enriched as (
    select
      ...
      (
        nullif(btrim(coalesce(c.matricula_tractora, '')), '') is not null
        or nullif(btrim(coalesce(c.matricula_remolque, '')), '') is not null
      ) as has_matricula_values,
      case
        when jsonb_typeof(c.change_meta) = 'object'
          and jsonb_typeof(c.change_meta -> '_match') = 'object'
          and coalesce(c.change_meta -> '_match' ->> 'matched_pedido_id', '') ~ '^[0-9]+$'
  ...
  classified as (
    select
      ...
      (
        e.has_header_meta
        and lower(e.header_action) = 'update'
      ...
      (
        ...
        or (
          (not e.has_header_meta or cardinality(e.header_columns) = 0)
          and e.has_matricula_values
        )
    ...
    select
      ...
      (
        ...
        or (
          ...
          and nullif(btrim(coalesce(c.referencia_cliente, '')), '') is not null
          and (
            select count(*)
            from public.pedidos p
            where p.tipo_pedido = 'P220'
              and p.referencia_cliente = c.referencia_cliente
              and (c.clienteid is null or p.clienteid = c.clienteid)
              and (
                c.sujetodomicilioid_destino is null
                or p.sujetodomicilioid_destino = c.sujetodomicilioid_destino
              )
          ) = 1
        )
        or (
...
comment on function public.get_cambios_group_page(
  integer, integer, text, text, bigint, bigint, text, text, text, text
)
is 'Devuelve una página de cambios agrupada por PDF sin partir grupos, incluyendo fila meta con totales.';
